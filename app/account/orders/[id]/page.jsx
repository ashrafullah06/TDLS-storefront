...
export async function GET(_req, { params }) {
  try {
    ...
    // 4) Lazy deps (Node only) – handle CJS/ESM interop safely
    ...
    // Barcode (orderNumber preferred)
    const codeText = String(order.orderNumber || order.id);
    ...
    drawBox(doc, () => {
      ...
      metaRow(doc, "Order Number", `#${String(order.orderNumber || order.id)}`);
...
function drawFocus(doc, rows) {
  ...
  // Move cursor below the strip (avoid overlap with next section)
  doc.y = y + h + 14;
}
...
function tableHeader(doc, headers, y) {
  ...
  doc.text(headers[2], 36 + cols[2], doc.y, {
    width: 40,
    align: "right",
  });
  doc.text(headers[3], 36 + cols[3], doc.y, {
    width: 52,
    align: "right",
  });
  doc.text(headers[4], 36 + cols[4], doc.y, {
    width: 72,
    align: "right",
  });
}
...
function tableRow(doc, y, cells) {
  ...
  doc.text(cells[2], 36 + cols[2], y, {
    width: 40,
    align: "right",
  });
  doc.text(cells[3], 36 + cols[3], y, {
    width: 52,
    align: "right",
  });
  doc.text(cells[4], 36 + cols[4], y, {
    width: 72,
    align: "right",
  });
  return y + 18;
