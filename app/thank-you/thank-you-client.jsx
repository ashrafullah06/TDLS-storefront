//FILE 2 (new): app/thank-you/thank-you-client.jsx
"use client";

/**
 * Thank You page (Client)
 *
 * URL shape we expect:
 *   /thank-you?order=<orderId>&paid=<0|1>
 *
 * Behavior:
 *  - Receives orderId & paid via props from the Server page.
...
export default function ThankYouClient({ orderId = "", paid = "" }) {
  const isPaid = String(paid) === "1";
      ...
      <main className="mx-auto" style={{ maxWidth: 1100, padding: "calc(var(--nav-h,80px) + 18px) 20px 80px" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
          ...
          <div className="space-y-8">
            ...
            {order?.shipping ? (
              <section className="rounded-xl p-3" style={{ border: `1px solid ${BORDER}`, background: "#fff" }}>
...
function normalizeOrder(raw) {
  ...
  const items = raw.items || raw.lines || raw.orderItems || [];
  ...
  const mappedItems = items.map((it) => ({
    ...
    subtotal:
      it.subtotal ||
      (Number(it.unitPrice || it.price || 0) * Number(it.quantity || it.qty || 1)),
    image:
      it.image ||
      it.thumbnail ||
      it.variant?.media?.[0]?.url ||
