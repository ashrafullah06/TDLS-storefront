...
async function getCustomerId(req) {
  try {
    ...
    let authOptions;
    try { ({ authOptions } = await import("@/lib/auth")); } catch {}
    try { ({ authOptions } = await import("@/app/api/auth/[...nextauth]/authOptions")); } catch {}
...
export async function GET(req) {
  ...
  try {
    ...
    return NextResponse.json({ error: "prefs unavailable", detail: String(e) }, { status: 503 });
  }
}
...
export async function POST(req) {
  ...
  const { changes = [] } = body || {};
  if (!Array.isArray(changes) || changes.length === 0) return NextResponse.json({ error: "changes required" }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ error: "update prefs failed", detail: String(e) }, { status: 503 });
  }
}
