...
const MEM =
  G.__TDLS_MENU_INDEX_MEM__ ??
  (G.__TDLS_MENU_INDEX_MEM__ = new Map());

const INFLIGHT =
  G.__TDLS_MENU_INDEX_INFLIGHT__ ??
  (G.__TDLS_MENU_INDEX_INFLIGHT__ = new Map());

const MEM_TTL_MS = (() => {
  const n = Number(
    process.env.TDLS_MENU_INDEX_TTL_MS ?? 60_000
  );
...
async function fetchViaProxy(
  ...
  /*
   * Normal menu loads MUST use the hardened /api/strapi cache.
   * Only an explicit ?refresh=1 request bypasses that cache.
   */
  if (forceFresh) {
    u.searchParams.set(
      "noCache",
...
function makeKindBucket() {
  return {
    ...
    counts: {
      ...
      productLinks: 0,
      productsUsed: 0,
      productsSkippedMissingTier:
        0,
      productsSkippedMissingAudience:
        0,
      productsSkippedMissingCategory:
        0,
    },
...
export async function GET(
      ...
      const n =
        Number(
          url.searchParams.get(
            ...
            process.env
              .TDLS_MENU_INDEX_PRODUCTS_PAGESIZE ??
            1500
        );
  ...
  /*
   * Include tierKind/debug in cache key because they alter output.
   */
  const cacheKey =
    `menu-index|v1|ps=${pageSize}` +
    `|tk=${forcedTierKind || "auto"}` +
    `|dbg=${debug ? "1" : "0"}`;
  ...
  const result =
    await runDedupe(
      ...
      async () => {
        const productsPath =
          `/products?pagination[pageSize]=${encodeURIComponent(
            String(
              pageSize
            )
          )}` +
          `&fields[0]=slug` +
          `&fields[1]=name` +
          `&fields[2]=disable_frontend` +
          `&fields[3]=is_archived` +
          `&fields[4]=status`;
        ...
        const payload = {
          ...
          counts: {
            ...
            uniqueAudiences:
              audiencesBySlug.size,

            uniqueCategories:
              categoriesBySlug.size,
          },
        };
