...
export async function GET() {
  try {
    const r = await fetch(`${BASE}/packages`, { method: "POST", headers: h(), body: JSON.stringify({}) });
    const data = await r.json();
    if (!r.ok) return NextResponse.json({ error: "packages failed", detail: data }, { status: r.status });
    return NextResponse.json({ packages: data });
  } catch (e) {
    return NextResponse.json({ error: "packages error", detail: String(e) }, { status: 500 });
  }
}
