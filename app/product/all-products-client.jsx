...
const CLIENT_PRODUCTS_PAGE_SIZE = 24;
const CLIENT_PRODUCTS_FETCH_CONCURRENCY = 1;
const CLIENT_PRODUCTS_MAX_PAGES = 1000;

function buildProductsClientPath(page, withCount = false) {
  const params = new URLSearchParams();

  params.set("sort", "id:asc");
  params.set("pagination[page]", String(page));
  params.set("pagination[pageSize]", String(CLIENT_PRODUCTS_PAGE_SIZE));
  params.set("pagination[withCount]", withCount ? "true" : "false");

  params.set("populate[image]", "*");
  params.set("populate[images]", "*");
  params.set("populate[gallery]", "*");

  params.set("populate[product_variants][populate][sizes]", "*");
  params.set("populate[product_variants][populate][image]", "*");
  params.set("populate[product_variants][populate][color]", "*");
  ...
  taxonomyRelations.forEach((relation) => {
...
function getClientProductKey(product) {
  return String(
    product?.id ??
      product?.documentId ??
      product?.slug ??
...
function mergeUniqueClientProducts(target, incoming) {
  const out = Array.isArray(target) ? target : [];
  const seen = new Set(out.map(getClientProductKey).filter(Boolean));

  (Array.isArray(incoming) ? incoming : []).forEach((product) => {
    const key = getClientProductKey(product);
...
function readClientPageCount(payload) {
  const pagination = payload?.meta?.pagination || {};
  const pageCount = Number(pagination.pageCount);

  if (Number.isFinite(pageCount) && pageCount > 0) {
    return Math.min(
      CLIENT_PRODUCTS_MAX_PAGES,
      Math.max(1, Math.floor(pageCount))
    );
  }

  const total = Number(pagination.total);
  const pageSize = Number(pagination.pageSize) || CLIENT_PRODUCTS_PAGE_SIZE;
...
async function fetchProductsClientPage(page, withCount = false) {
  ...
  if (!Array.isArray(payload?.data)) {
    if (Array.isArray(payload)) {
      return {
        products: payload.map(normalizeFetchedProduct),
        pageCount: 1,
      };
    }
  ...
  return {
    products: payload.data.map(normalizeFetchedProduct),
    pageCount: readClientPageCount(payload),
  };
}
...
async function fetchProductsClient(
  ...
  const first =
    Array.isArray(seed) && Number.isFinite(pageCountHint)
  ...
  const pageCount = Math.min(
    CLIENT_PRODUCTS_MAX_PAGES,
    Math.max(1, Number(first.pageCount || 1))
  );

  if (pageCount <= 1) return allProducts;

  for (
    let startPage = 2;
    startPage <= pageCount;
    startPage += CLIENT_PRODUCTS_FETCH_CONCURRENCY
  ) {
    if (isCancelled()) return allProducts;

    const pages = [];

    for (
      let offset = 0;
      offset < CLIENT_PRODUCTS_FETCH_CONCURRENCY;
      offset++
    ) {
    ...
    const results = await Promise.all(
      pages.map((page) => fetchProductsClientPage(page, false))
    );
    ...
    if (results.some((result) => !result)) {
      console.error(
        "[products] A catalogue page could not be loaded; keeping loaded cards."
      );
      return allProducts;
    }
    ...
    results.forEach((result) => {
      if (result?.products) {
        mergeUniqueClientProducts(allProducts, result.products);
      }
    });
...
const getSlugs = (rel) => {
  ...
  const one =
    d?.attributes?.slug || d?.slug || d?.attributes?.name || d?.name;
...
const hasSlug = (rel, slug) => {
  ...
  return getSlugs(rel).map(normSlug).includes(target);
};
  ...
  for (const raw of rawList) {
    ...
    if (sizes && sizes.length) {
      sizes.forEach((s) => {
        ...
        if (!merged.size_name) {
          merged.size_name =
            s.size_name || s.size || s.label || s.value || null;
        }

        if (merged.price == null && merged.sale_price == null) {
          if (typeof s.effective_price === "number") {
            merged.price = s.effective_price;
          } else if (typeof s.price === "number") {
            merged.price = s.price;
          } else if (typeof s.price_override === "number") {
            merged.price = s.price_override;
          }
        }
        ...
        if (merged.stock == null && merged.stock_quantity == null) {
          if (typeof s.stock_quantity === "number") {
            merged.stock = s.stock_quantity;
          }
        }
...
const deriveTypes = (p) => {
  ...
  const fromSlugs = arr(p?.categories_slugs || A?.categories_slugs).map(
    normSlug
  );
...
const deriveTags = (p) => {
  ...
  const events = getSlugs(p?.events_products_collections).concat(
    getSlugs(A?.events_products_collections)
  );

  const audience = getSlugs(p?.audience_categories).concat(
    getSlugs(A?.audience_categories)
  );

  const eventsSlugs = arr(
    p?.events_products_collections_slugs ||
      A?.events_products_collections_slugs
  );

  const audienceSlugs = arr(
    p?.audience_categories_slugs || A?.audience_categories_slugs
  );
...
const deriveUnitMetrics = (p) => {
  ...
  const volume_ml =
    ...
    Number(
      A?.volume_ml ??
        p?.volume_ml ??
        (typeof A?.volume_l === "number"
          ? A.volume_l * 1000
          : undefined) ??
        (typeof p?.volume_l === "number"
          ? p.volume_l * 1000
          : undefined)
    ) ||
    null;
...
export default function AllProductsClient({
  ...
  const resolvedBaseUrl = useMemo(
    () => (siteBaseUrl || clientOrigin || "").toString().replace(/\/+$/, ""),
  ...
  useEffect(() => {
    ...
    const onResize = () =>
      setViewportW(Math.max(280, Math.min(820, window.innerWidth || 390)));
  ...
  useEffect(() => {
    ...
    if (Array.isArray(products) && initialPageCount === 1) return;
    if (products?.length && !Number.isFinite(initialPageCount)) return;

    void fetchProductsClient(
      products,
      initialPageCount,
      (list) => {
        if (!cancelled) setClientProducts(list);
      },
      () => cancelled
    ).catch((error) =>
      console.error("[products] Catalogue load failed:", error.message)
    );

    return () => {
      cancelled = true;
    };
  }, [products, initialPageCount]);
  ...
  const facets = useMemo(() => {
    ...
    globallyFiltered.forEach((p) => {
      ...
      v.forEach(
        (x) => x.color_name && inc(colors, normSlug(String(x.color_name)))
      );
  ...
  useEffect(() => {
    ...
    setMaxPrice((current) =>
      current === 0 || current === previous.max
        ? facets.priceMax || 0
        : current
    );

    setMinPrice((current) =>
      current === 0 || current === previous.min
        ? facets.priceMin || 0
        : current
    );

    previousPriceBounds.current = {
      min: facets.priceMin || 0,
      max: facets.priceMax || 0,
    };
  ...
  const activeChips = useMemo(() => {
    ...
    if (minDiscount > 0) {
      chips.push({ k: "discount", v: `${minDiscount}%+` });
    }
    ...
    [...sizeSet].forEach((s) =>
      chips.push({ k: "size", v: s, val: s })
    );
    [...materialSet].forEach((m) =>
      chips.push({ k: "mat", v: pretty(m), val: m })
    );
    [...scentSet].forEach((f) =>
      chips.push({ k: "scent", v: pretty(f), val: f })
    );
    [...originSet].forEach((o) =>
      chips.push({ k: "origin", v: pretty(o), val: o })
    );
    [...dietarySet].forEach((d) =>
      chips.push({ k: "diet", v: pretty(d), val: d })
    );
  ...
  const closeInlineFilters = useCallback(
    (asCTA = true) => {
      ...
      if (asCTA && changed) {
        window.setTimeout(() => scrollToResults(), 160);
      }

      window.setTimeout(() => setFiltersMounted(false), 340);
    },
  ...
  const pageFiltered = useMemo(() => {
    ...
    const getMeta = (p) => {
      ...
      variants.forEach(
        (x) => x.color_name && colors.add(normSlug(String(x.color_name)))
      );
    ...
    if (deptSel) {
      list = list.filter((p) =>
        meta.get(p).departments.includes(deptSel)
      );
    }
    if (materialSet.size) {
      list = list.filter((p) =>
        meta.get(p).mats.some((m) => materialSet.has(m))
      );
    }
    if (scentSet.size) {
      list = list.filter((p) =>
        meta.get(p).scents.some((s) => scentSet.has(s))
      );
    }
    if (originSet.size) {
      list = list.filter((p) =>
        meta.get(p).origins.some((o) => originSet.has(o))
      );
    }
    if (dietarySet.size) {
      list = list.filter((p) =>
        meta.get(p).diet.some((d) => dietarySet.has(d))
      );
    }
    if (flagImported) {
      list = list.filter((p) => meta.get(p).flags.imported);
    }
    if (flagGiftable) {
      list = list.filter((p) => meta.get(p).flags.giftable);
    }
    if (minRating > 0) {
      list = list.filter((p) => meta.get(p).rating >= minRating);
    }
    ...
    if (onlyInStock) {
      list = list.filter((p) => meta.get(p).inStock);
    }
    if (minDiscount > 0) {
  ...
  // The sentinel can appear after progressively loaded products arrive.
  // Reconnect whenever its presence or position changes.
  useEffect(() => {
    const node = sentinelRef.current;
    if (!node || visibleCount >= pageFiltered.length) return;
  ...
  useEffect(() => {
    ...
    try {
      ...
      try {
        const raw =
          window.localStorage.getItem("recently_viewed_v1") || "[]";
      ...
      recent.forEach((r) => {
        if (r?.selectedColor) {
          interestColors.add(String(r.selectedColor));
        }
  ...
  const S = useMemo(() => {
    ...
    return {
      ...
      taglineRow: {
        ...
        background:
          "linear-gradient(135deg,rgba(15,33,71,.06),rgba(243,244,246,1))",
      ...
      miniBar: {
        ...
        background:
          "linear-gradient(135deg,#ffffff 0%,#f9fafb 45%,#edf2ff 100%)",
      ...
      chip: (active) => ({
        ...
        background: active
          ? "linear-gradient(135deg,#0f2147,#111827)"
          : "#ffffff",
      ...
      activeChip: {
        ...
        border: "1px solid rgba(15,33,71,.12)",
        background:
          "linear-gradient(135deg,rgba(15,33,71,1),rgba(17,24,39,1))",
        color: "#f9fafb",
        fontWeight: 800,
        display: "inline-flex",
        alignItems: "center",
        ...
        <div style={S.rail} aria-label="Filter controls rail">
          ...
          <div style={S.railGroup}>
            ...
            >
              ...
              {facets.hasPPU && (
                <option value="ppu-asc">Unit price ↑</option>
              )}
            </select>
          </div>
        ...
        <div style={S.container}>
          ...
          <section style={S.taglineRow}>
            <div style={S.taglineMeta}>
              <div>
                Total catalogue{" "}
                <span style={S.taglineMetaHighlight}>{total}</span>
              </div>
            ...
            {!isMobile ? (
              <div style={S.desktopRow}>
                ...
                >
                  ...
                  <option value="best-rated">Best rated</option>
                  {facets.hasPPU && (
                    <option value="ppu-asc">Price per unit ↑</option>
                  )}
                </select>
          ...
          {filtersMounted && (
            ...
            >
              <div style={S.inlinePanel}>
                ...
                <div style={S.inlineBody}>
                  {renderFullFilterControls(isMobile)}
                </div>
          ...
          >
            Showing {Math.min(visibleCount, total)} of {total} item
            {total === 1 ? "" : "s"}
          </div>
          ...
          {total === 0 ? (
            ...
            <ul style={S.grid}>
              {visible.map((p, idx) => (
                <li
                  key={p.id ?? p.slug ?? p?.attributes?.slug ?? `idx-${idx}`}
                >
                  <ProductCard
                    product={p}
            ...
            <>
              ...
              >
                {suggestions.map((p, idx) => (
                  <li
                    key={`sugg-${
                      p.id ?? p.slug ?? p?.attributes?.slug ?? idx
                    }`}
                  >
                    <ProductCard
                      product={p}
