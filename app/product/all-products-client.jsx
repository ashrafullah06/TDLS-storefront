...
const deriveTags = (p) => {
  ...
  const eventsSlugs = arr(
    p?.events_products_collections_slugs || A?.events_products_collections_slugs
  );
...
export default function AllProductsClient({ products }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => entries.forEach((e) => e.isIntersecting && setVisibleCount((c) => c + 24)),
  ...
  const pageFiltered = useMemo(() => {
    ...
    if (deptSel)
      list = list.filter((p) => meta.get(p).departments.includes(deptSel));
    if (materialSet.size)
      list = list.filter((p) =>
  ...
  const S = {
    ...
    topBar: {
      ...
      borderRadius: 18,
      background:
        "linear-gradient(135deg,#ffffff 0%,#f9fafb 40%,#edf2ff 100%)",
      backdropFilter: "saturate(180%) blur(8px)",
      boxShadow: "0 18px 40px rgba(15,33,71,.10)",
      zIndex: 5,
    },
    group: {
      display: "flex",
    ...
    // Minimal floating icons dock (cart + wishlist)
    iconDock: {
      position: "fixed",
    ...
    iconBtn: {
      pointerEvents: "auto",
      width: 52,            // ⬆️ increased from 44 for a bigger target
      height: 52,           // ⬆️ increased from 44
      borderRadius: "50%",
      border: "1px solid rgba(15,33,71,.18)",
      background: "#ffffff",
      display: "flex",
    ...
    iconBadge: {
      position: "absolute",
      top: -6,              // slightly adjusted for larger badge
      right: -6,
      minWidth: 22,         // ⬆️ bigger number badge
      height: 22,           // ⬆️ bigger number badge
      borderRadius: 999,
      background: "#0f2147",
      color: "#f9fafb",
      fontSize: 11,         // ⬆️ larger text
      fontWeight: 800,
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
  ...
  return (
    <>
      ...
      <div style={S.iconDock} aria-label="Cart and wishlist">
        ...
        >
          <FaShoppingCart size={28} color="#0c2340" />
          {cartCount > 0 && (
        ...
        >
          <FaRegHeart size={24} color="#7a102b" />
        </button>
      </div>

      {/* Full-width panel now driven by .all-products-page */}
      <main className="all-products-page" style={S.page}>
          ...
          <div style={S.titleLeft}>
            ...
            <div>
              ...
              <div style={S.subtitle}>
                CURATED BY THE DNA LAB CLOTHING
              </div>
            </div>
          </div>
        </div>
        ...
        <section style={S.taglineRow}>
          <div style={S.taglineLeft}>
            <div style={S.taglineTitle}>House Of Tdlc</div>
            <p style={S.taglineCopy}>
              <span style={S.taglineAccent}>
                “Simplicity is the ultimate sophistication.”
              </span>{" "}
              Explore wardrobe foundations, limited drops and future
              signatures from THE DNA LAB CLOTHING – edited to feel calm,
              precise and quietly luxurious.
            </p>
          </div>
        ...
        {/* Top filters (sticky) */}
        <div style={S.topBar} role="region" aria-label="Filters">
          {/* Search */}
          <div style={S.group}>
          ...
          <div style={S.group}>
            ...
            >
              ...
              {facets.hasPPU && (
                <option value="ppu-asc">Price per unit: Low → High</option>
              )}
            </select>
          </div>
          ...
          <>
            ...
            >
              {suggestions.map((p, idx) => (
                <li
                  key={`sugg-${p.id ?? p.slug ?? p?.attributes?.slug ?? idx}`}
                >
                  <ProductCard
                    product={p}
                    onQuickView={(prod) => {
