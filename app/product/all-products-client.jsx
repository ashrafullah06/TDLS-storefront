...
const CLIENT_PRODUCTS_FETCH_CONCURRENCY = 3;
...
function buildProductsClientPath(page, withCount = false) {
  ...
  // Taxonomy relations used by the existing All Products filters.
...
async function fetchFromStrapi(path, noCache = false) {
  try {
    ...
    const res = await fetch(`/api/strapi?${query.toString()}`, {
      method: "GET",
...
async function fetchProductsClientPage(page, withCount = false) {
  ...
  // One direct retry bypassing proxy caches if the first attempt failed.
  if (!json) {
  ...
  if (!Array.isArray(payload?.data)) {
    // Very defensive: tolerate an already-unwrapped array.
...
async function fetchProductsClient() {
  /*
   * This is an emergency browser fallback only. The normal path receives the
   * complete catalog from app/product/page.jsx.
   *
   * Keep the same behavior here without recreating the old single oversized
   * /products?populate=* request.
   */
  const first = await fetchProductsClientPage(1, true);
  if (!first) return [];
  ...
  for (
    ...
    for (let offset = 0; offset < CLIENT_PRODUCTS_FETCH_CONCURRENCY; offset++) {
...
const inStockOf = (p) => {
  ...
  const varHas = normVariants(p).some(
    (v) => (v.stock || 0) > 0
  );
...
const deriveTypes = (p) => {
  ...
  const fromRel = getSlugs(p?.categories).concat(
...
const deriveTags = (p) => {
  ...
  const events = getSlugs(
    p?.events_products_collections
  ).concat(
    getSlugs(
      A?.events_products_collections
    )
  );

  const audience = getSlugs(
    p?.audience_categories
  ).concat(
...
const deriveDietary = (p) => {
  ...
  const pushIf = (cond, slug) =>
    cond && flags.push(slug);

  pushIf(A?.vegan || p?.vegan, "vegan");
  pushIf(A?.halal || p?.halal, "halal");
  pushIf(
    A?.vegetarian || p?.vegetarian,
    "vegetarian"
  );
  pushIf(A?.organic || p?.organic, "organic");
  pushIf(
    A?.gluten_free || p?.gluten_free,
    "gluten-free"
  );
  pushIf(
    A?.sugar_free || p?.sugar_free,
    "sugar-free"
  );
  pushIf(
    A?.lactose_free || p?.lactose_free,
    "lactose-free"
  );
...
const deriveUnitMetrics = (p) => {
  ...
  const vWeight =
    v.find((x) => x.weight_g)?.weight_g ||
    null;

  const vVol =
    v.find((x) => x.volume_ml)?.volume_ml ||
    null;
...
export default function AllProductsClient({
  ...
  const [clientProducts, setClientProducts] =
    useState(null);
  ...
  const [scentSet, setScentSet] = useState(
    () => new Set()
  );

  const [originSet, setOriginSet] = useState(
    () => new Set()
  );

  const [dietarySet, setDietarySet] = useState(
    () => new Set()
  );
  ...
  const facets = useMemo(() => {
    ...
    globallyFiltered.forEach((p) => {
      ...
      fallbackColors(p).forEach((c) =>
        inc(colors, normSlug(c))
      );

      fallbackSizes(p).forEach((s) =>
        inc(sizes, s)
      );
      ...
      deriveDepartments(p).forEach((d) =>
        inc(departments, d)
      );

      deriveMaterial(p).forEach((m) =>
        inc(materials, m)
      );

      deriveScentFlavor(p).forEach((s) =>
        inc(scents, s)
      );

      deriveOrigin(p).forEach((o) =>
        inc(origins, o)
      );

      deriveDietary(p).forEach((d) =>
        inc(dietary, d)
      );
  ...
  const S = useMemo(() => {
    ...
    return {
      ...
      grid: {
        ...
        gridTemplateColumns:
          `repeat(auto-fill,minmax(${scale.gridMin}px,1fr))`,
  ...
  const renderFullFilterControls = (
    forMobileCtx
  ) => {
    const mobileCtx =
      !!forMobileCtx;
    ...
    return (
      <>
        <div
          style={S.rail}
          aria-label="Filter controls rail"
        >
          {/* Search */}
          <div
            style={{
