...
function fetchWithTimeout(url, options = {}, ms = FETCH_TIMEOUT_MS) {
  return timedFetch(url, options, ms);
}
...
function unwrapAndFlatten(raw) {
  ...
  const meta =
    payload?.meta?.pagination || payload?.meta?.pagination || null;
  ...
  const pagination =
    payload?.meta?.pagination || payload?.meta?.pagination;
  ...
  base.forEach((v) => {
    ...
    if (sizes && sizes.length) {
      ...
      if (price == null && v?.price_range) {
        if (typeof v.price_range.min === "number") {
          price = v.price_range.min;
...
export default function CollectionsSegmentClient({
  ...
  useLayoutEffect(() => {
    try {
      ...
      return () => {
        if (prev == null) {
          root.removeAttribute("data-tdls-no-debug-grid");
        } else {
          root.setAttribute("data-tdls-no-debug-grid", prev);
        }
      };
    } catch {
      // Keep rendering if document access is unavailable.
    }
  }, []);
  ...
  const buildServerFiltersQS = useCallback(() => {
    ...
    const addRel = (rel, slug) => {
      ...
      p.set(`filters[${rel}][slug][$eq]`, normSlug(slug));
    };
    ...
    if (isNonEmpty(selectedEvent)) {
      addRel("events_products_collections", selectedEvent);
    }
    ...
    if (isNonEmpty(selectedTier)) {
      ...
      tierRelations.forEach((rel, index) => {
        p.set(`filters[$or][${index}][${rel}][slug][$eq]`, t);
      });
    }
  ...
  const globallyFiltered = useMemo(() => {
    ...
    if (fetchMode === "serverFiltered") {
      return rawProducts.filter((p) => p && !p.disable_frontend);
    }

    return rawProducts.filter(matchesCollection);
  }, [rawProducts, matchesCollection, fetchMode]);
  ...
  useEffect(() => {
    try {
      ...
      // Ignore unavailable or invalid saved filters.
    }
  }, [FILTERS_STORAGE_KEY]);
  ...
  const activeChips = useMemo(() => {
    ...
    if (isNonEmpty(search)) {
      chips.push({ k: "search", v: search });
    }

    if (typeSel) {
      chips.push({ k: "type", v: pretty(typeSel) });
    }

    if (brandSel) {
      chips.push({ k: "brand", v: pretty(brandSel) });
    }

    if (onlyInStock) {
      chips.push({ k: "stock", v: "In Stock" });
    }
  ...
  const visible = useMemo(
    () => pageFiltered.slice(0, displayTarget),
    [pageFiltered, displayTarget]
  );
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (pageFiltered.length >= displayTarget) return;

      const nextPage = (remotePage || 0) + 1;
      if (nextPage > remotePageCount) return;
  ...
  const headerTitle = selectedAudience
  ...
  return (
    <div style={S.pageShell}>
      ...
      <main style={S.pageInner}>
        ...
        <div style={S.count}>
          Showing {Math.min(displayTarget, pageFiltered.length)} of{" "}
          {totalText} curated item{totalText === 1 ? "" : "s"}
        </div>
        ...
        <div style={S.gridCard}>
          {loadingInitial && pageFiltered.length === 0 ? (
            ...
            <>
              ...
              {!!fetchError && (
                <div style={{ ...S.loadingRow, color: "#7a102b" }}>
                  {fetchError}
                </div>
              )}
            </>
