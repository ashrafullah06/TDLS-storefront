//✅ FILE 2: src/components/homepage/homepage-client.inner.jsx
"use client";
...
function resolveCTA({ slide, slotIndex }) {
  ...
  const pickedLabel = (
    labelCandidates.find((v) => typeof v === "string" && v.trim()) || fallback.label
  )
    .toString()
    .trim();
...
/* ========================= Media Tile ========================= */
function MediaTile({ slide, mode = "grid", active, eager = false, sizesHint = null, forceCover = false, focal = "center" }) {
  ...
  useEffect(() => {
    ...
    if (!el || !hls) return;
    const native = typeof el.canPlayType === "function" && el.canPlayType("application/vnd.apple.mpegURL");
    if (native) {
      el.src = hls;
      return;
    }
...
function FullBleedSingle({ slides, cta, eagerFirst = false, onBlankClick }) {
  ...
  return (
    <section
      ...
      style={{ background: "#fff", borderRadius: 0, boxShadow: "0 10px 64px #e5e5e5cc" }}
  ...
  return (
    <section className="full-bleed" style={{ background: "#fff", boxSizing: "border-box" }}>
      ...
      <style>{`
        ...
        @media (min-width: 1440px) { .hero-grid { gap: 96px; } }
...
function coerceSlides(val) {
  ...
  if (val.slides && !Array.isArray(val.slides)) {
    ...
    const hasMedia = imgUrlOf(node) || videoUrlsOf(node).mp4 || videoUrlsOf(node).hls || videoUrlsOf(node).webm;
...
function planRows(groups) {
  const rows = [];
  const main = groups[0];
  if (main?.slides) rows.push({ type: "single", items: [{ slides: main.slides, slotIndex: 0 }] });
  ...
  while (i < rest.length) {
    ...
    if (chunk.length) rows.push({ type: "grid", columns: take === 2 ? 2 : 4, items: chunk });
    i += take;
    p += 1;
  }
  return rows;
...
/* ========================= Head injection (client-only) ========================= */
function useHomepageHeadLinks({ apiBase, firstHeroImage }) {
  useEffect(() => {
    if (typeof document === "undefined") return;

    const prev = document.head.querySelectorAll('link[data-tdls-home-head="1"]');
    ...
    if (apiBase) {
      ...
      l.setAttribute("href", apiBase);
      l.setAttribute("crossorigin", "");
      l.setAttribute("data-tdls-home-head", "1");
    ...
    if (firstHeroImage) {
      ...
      l.setAttribute("href", firstHeroImage);
  ...
  }, [apiBase, firstHeroImage]);
}

/* ========================= Main ========================= */
export default function HomepageClientInner({ homepage: initialHomepage = null, error: initialError = null }) {
  ...
  useHomepageHeadLinks({ apiBase: API_BASE, firstHeroImage });
  ...
  return (
    <div
      ...
      style={{ paddingBottom: "calc(var(--bottom-bar-h, 64px) + var(--safe-bottom) + 18px)" }}
    >
      <style jsx global>{`
        :root {
        ...
        @media (min-width: 1024px) { :root { --hero-section-gap: 80px; } }
        @media (min-width: 1440px) { :root { --hero-section-gap: 112px; } }
        @media (max-height: 480px) { :root { --hero-h: clamp(220px, 74svh, var(--hero-max-h, 520px)); } }

        .tdls-bigmark-outer{ isolation: isolate; }
        ...
        @media (prefers-reduced-motion: reduce){
          .tdls-bigmark-outer::before,
          .tdls-bigmark-outer::after,
          .tdls-bigmark-inner{ animation: none !important; }
        }
      `}</style>
