...
/* -------------------------------------------------------------------------- */
/* PROMOBAR / NAVBAR STACK CONTRACT                                            */
/* -------------------------------------------------------------------------- */

const PROMO_HEIGHT_VAR = "--tdls-promobar-h";

const NAVBAR_HEIGHT_VAR = "--tdls-navbar-h";

const HEADER_STACK_HEIGHT_VAR = "--tdls-header-stack-h";
 ...
 * PromoBar = 2147483647
 *
 * Navbar must remain immediately below it.
 */
const NAVBAR_Z_INDEX = 2147483000;

/* -------------------------------------------------------------------------- */
/* HELPERS                                                                     */
/* -------------------------------------------------------------------------- */

function readCssPx(element, property, fallback = 0) {
  try {
    ...
    >
      <span
        ...
        style={{
          ...
          transition:
            "all .18s cubic-bezier(.7,.1,.8,1.2)",
        }}
      >
        <svg
...
/**
 * All hooks always run when this component is mounted.
 * It is only mounted for non-admin routes.
 */
function NavbarInner({
  ...
  const handleMenuClick = async () => {
    if (!menuOpen && categories.length === 0) {
      setCategories(
        await fetchCategoriesFromStrapi()
      );
    }
  ...
  useEffect(() => {
    ...
    const LEGACY_ABBR_LO = LEGACY_ABBR.toLowerCase();
    ...
    const normalizeText = (input) => {
      ...
      out = out.replace(
        new RegExp(
          escapeRegExp(LEGACY_ABBR_LO),
          "g"
        ),
        NEW_ABBR.toLowerCase()
      );

      out = out.replace(
        new RegExp(
          escapeRegExp(LEGACY_LONG),
          "gi"
        ),
        NEW_LONG
      );
    ...
    const selectorsFor = (token) => [
      ...
      `[data-big-${token}]`,
      `[data-${token}-splash]`,
      `[data-show-big-${token}='true']`,
      `[aria-modal='true'][data-${token}]`,
    ];

    const SELECTORS = [
      ...selectorsFor(newLower),
      ...selectorsFor(legacyLower),
    ];
  ...
  /* ---------------------------------------------------------------------- */
  /* NAVBAR + PROMOBAR DYNAMIC HEADER STACK                                 */
  /* ---------------------------------------------------------------------- */
  ...
  useEffect(() => {
    ...
    const previousBodyPaddingTop =
      body.style.paddingTop;

    const previousNavH =
      root.style.getPropertyValue("--nav-h");

    const previousNavbarH =
      root.style.getPropertyValue(
        NAVBAR_HEIGHT_VAR
      );

    const previousStackH =
      root.style.getPropertyValue(
        HEADER_STACK_HEIGHT_VAR
      );
    ...
    const applyHeights = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        /*
         * Existing components that use --nav-h should remain below
         * the complete header stack.
         */
        root.style.setProperty(
          "--nav-h",
          `${totalHeight}px`
    ...
    if (
      ...
      observer = new ResizeObserver(applyHeights);
  ...
  return (
    <>
       ...
       * CRITICAL:
       *
       * PromoBar is now mounted by the storefront header itself.
       * It therefore cannot disappear merely because a layout forgot
       * to include <Promobar />.
       */}
      <Promobar />
      ...
      <header
        ref={headerRef}
        data-tdls-navbar="true"
        className="tdls-header"
        style={{
          position: "fixed",

          /*
           * Always directly underneath PromoBar.
           */
          top:
            "var(--tdls-promobar-h, 0px)",
          ...
          paddingLeft:
            "var(--nav-gutter-x, var(--page-gutter-x))",

          paddingRight:
            "var(--nav-gutter-x, var(--page-gutter-x))",

          transition:
            "top .18s ease, background .34s, box-shadow .28s, border .28s, height .18s",
...
/**
 * - Always calls usePathname.
 * - Admin does not receive storefront Navbar or PromoBar.
 */
