// FILE: src/lib/strapi-proxy.js

import "server-only";

import { NextResponse } from "next/server";
import { normalizeProductFilters } from "./product-query";
import { timedFetch, waitWithin } from "./timed-fetch";
...
// Allow local next start with a local Strapi instance.
// Enforce the production origin guard on Vercel.
...
const UPSTREAM_TIMEOUT_MS = (() => {
  const n = Number(
    process.env.STRAPI_PROXY_TIMEOUT_MS || 20000
  );
...
const HEAVY_GUARD_ENABLED =
  String(
    process.env.TDLS_STRAPI_HEAVY_GUARD ?? "1"
  )
    .trim()
    .toLowerCase() !== "0";
...
const HEAVY_MAX_QUERY_CHARS = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_HEAVY_MAX_QUERY_CHARS ??
      1400
  );
  ...
  return Math.min(
    6000,
    Math.max(400, Math.round(n))
  );
})();

const HEAVY_MAX_POPULATE_KEYS = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_HEAVY_MAX_POPULATE_KEYS ??
      40
  );
...
const PRODUCT_CACHE_CONTROL = (() => {
  const maxAge = Number(
    process.env.TDLS_STRAPI_PRODUCT_MAXAGE_SEC ?? 15
  );

  const sMaxAge = Number(
    process.env.TDLS_STRAPI_PRODUCT_SMAXAGE_SEC ?? 180
  );

  const swr = Number(
    process.env.TDLS_STRAPI_PRODUCT_SWR_SEC ?? 3600
  );

  const sie = Number(
    process.env.TDLS_STRAPI_PRODUCT_SIE_SEC ?? 86400
  );
...
// Retain a configuration error for the handler instead of
// throwing during module initialization.
let STRAPI_ORIGIN = "";
let STRAPI_API_BASE = "";
...
function isSafeLiteProductsTaxonomyRequest(path) {
  ...
  if (
    popCount >
    Math.min(30, HEAVY_MAX_POPULATE_KEYS)
  ) {
    return false;
  }
...
function isHeavyPopulateRequest(path) {
  if (isSafeLiteProductsTaxonomyRequest(path)) {
    return false;
  }
  ...
  if (search.length > HEAVY_MAX_QUERY_CHARS) {
    return true;
  }
  ...
  if (popCount > HEAVY_MAX_POPULATE_KEYS) {
    return true;
  }
  ...
  if (
    ...
    popCount >
      Math.max(
        10,
        Math.floor(HEAVY_MAX_POPULATE_KEYS / 2)
      )
  ) {
    return true;
  }
...
const BROAD_FALLBACK_PAGESIZE = (() => {
  ...
  return Math.min(
    DEFAULT_PRODUCTS_PAGESIZE,
    Math.max(50, Math.round(n))
  );
})();

/* ---------------- Product taxonomy profiles ---------------- */

const TAXONOMY_RELS = [
  ["audience_categories", ["slug", "name"]],
  ["categories", ["slug", "name"]],
  ["sub_categories", ["slug", "name"]],
  ["age_groups", ["slug", "name"]],
  ["gender_groups", ["slug", "name"]],
  ["events_products_collections", ["slug", "name"]],
];

const TAXONOMY_REL_SET = new Set(
  TAXONOMY_RELS.map((x) => x[0])
);
...
const FAST_META_DEFAULT_PAGE_SIZE = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_META_PAGESIZE ?? 1000
  );
...
function applyMetaFilterSafePopulate(
  ...
  for (const [rel, fields] of rels) {
    if (hasPopulateForRel(params, rel)) {
      continue;
    }
...
function applyFastMetaProfile(pathname, params) {
  ...
  if (!params.get("pagination[pageSize]")) {
    params.set(
      "pagination[pageSize]",
      String(FAST_META_DEFAULT_PAGE_SIZE)
    );
  }
...
function normalizeMetaPath(
  ...
  if (!FAST_META_PROFILES.has(pathname)) {
    return p;
  }
...
function applyProductsCardLitePopulate(params) {
  ...
  if (!hasPopulateForRel(params, "images")) {
...
/* ---------------- Schema compatibility retry ---------------- */

function buildProductsCompatibilityPath(p) {
  const { pathname, search } = splitPathAndQuery(p);
...
/* ---------------- Upstream URL and timeout ---------------- */

function buildTargetUrl(normalizedPath) {
  const { pathname, search } =
    splitPathAndQuery(normalizedPath);
...
function fetchWithTimeout(
  ...
  if (remaining <= 0) {
    const error = new Error(
      "Product upstream deadline exceeded"
    );
...
async function getPrisma() {
  ...
  _prismaPromise = import("@/lib/prisma").then(
    (m) => m?.default ?? m
  );
...
const INFLIGHT =
  ...
  (
    G.__TDLS_STRAPI_INFLIGHT__ = new Map()
  );

const MEM_META =
  G.__TDLS_STRAPI_MEM_META__ ??
  (
    G.__TDLS_STRAPI_MEM_META__ = new Map()
  );

const MEM_PROD =
  G.__TDLS_STRAPI_MEM_PROD__ ??
  (
    G.__TDLS_STRAPI_MEM_PROD__ = new Map()
  );

const LAST_GOOD =
  G.__TDLS_STRAPI_LAST_GOOD__ ??
  (
    G.__TDLS_STRAPI_LAST_GOOD__ = new Map()
  );

const STOCK_CACHE =
  G.__TDLS_STRAPI_STOCK__ ??
  (
    G.__TDLS_STRAPI_STOCK__ = new Map()
  );
...
const MEM_MAX_BYTES_META = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_MEMCACHE_MAX_BYTES_META ||
      1024 * 1024
  );
...
const MEM_MAX_BYTES_PROD = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_MEMCACHE_MAX_BYTES_PROD ||
      8 * 1024 * 1024
  );
...
const LAST_GOOD_MAX_BYTES = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_LASTGOOD_MAX_BYTES ||
      24 * 1024 * 1024
  );
...
const MEM_TTL_MS = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_MEMCACHE_TTL_MS ||
      120000
  );
...
const MEM_PROD_TTL_MS = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_MEMCACHE_PRODUCTS_TTL_MS ||
      45000
  );
...
const LAST_GOOD_TTL_MS = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_LASTGOOD_TTL_MS ||
      180000
  );
...
const LAST_GOOD_ANY_TTL_MS = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_LASTGOOD_ANY_TTL_MS ||
      600000
  );
...
const STOCK_CACHE_TTL_MS = (() => {
  const n = Number(
    process.env.TDLS_STOCKCACHE_TTL_MS ||
      300000
  );
...
function lastGoodSet(
  ...
  if (payloadStr.length > LAST_GOOD_MAX_BYTES) {
    return;
  }
...
function stockCacheSet(sizeId, stock) {
  ...
  STOCK_CACHE.set(sizeId, {
    exp: Date.now() + STOCK_CACHE_TTL_MS,
    stock: s,
  });
}
...
/* ---------------- Metadata cache warming helper ---------------- */

// Retained helper. Product requests no longer invoke it.
const WARM_STATE =
...
const WARM_TTL_MS = (() => {
  const n = Number(
    process.env.TDLS_STRAPI_WARM_TTL_MS ||
      10 * 60 * 1000
  );
...
async function warmMetaCachesIfNeeded(baseHeaders) {
  ...
  await Promise.allSettled(
    paths.map(async (pathname) => {
      ...
      if (
        memGet(MEM_META, cacheKey)?.payloadStr
      ) {
        return;
      }
      ...
      const dk =
        `meta|${eff}|pub|${
          STRAPI_TOKEN ? "tok" : "notok"
        }|nc0|g0`;

      await runDedupe(dk, async () => {
        const res = await fetchUpstreamResilient(
          target,
          baseHeaders,
          {
            delays: RETRY_DELAYS_META,
          }
        );
...
async function getStockMapForSizeIds(sizeIds) {
  ...
  try {
    ...
    const prismaVariants = await waitWithin(
    ...
    for (const v of variants) {
      ...
      for (const s of sizes) {
        ...
        const hasBool =
          ...
          (
            sAttrs &&
            typeof sAttrs.is_available === "boolean"
          );
...
async function patchProductsWithPrismaStock(
  ...
  const { items, sizeIds } =
    collectSizeIdsFromStrapiProducts(strapiData);
  ...
  const { bySizeId, source, error } =
    await getStockMapForSizeIds(sizeIds);
...
// Retained compatibility helper. The main handler does not use
// generic product payloads to replace a different request.
function anyProductsListPayloadStr(profile) {
  const key =
    profile === "filtersafe"
...
// Retained compatibility helper. Not invoked by product recovery
// in fetchStrapiProxy.
async function fetchBroaderProductsFallback(
  ...
  try {
    res = await fetchUpstreamResilient(
      target,
      baseHeaders,
      {
        deadlineAt,
        delays: RETRY_DELAYS_PRODUCTS,
      }
    );
  } catch {
    return null;
  ...
  if (
    ...
    const compatPath =
      buildProductsCompatibilityPath(fallbackPath);
...
export async function fetchStrapiProxy(req) {
  const t0 = Date.now();
  const deadlineAt = t0 + 25000;
  ...
  try {
    ...
    const allowHeavy =
      url.searchParams.get("allowHeavy") === "1";

    let effectivePath = canonicalizePath(
      normalizeProductsPath(
        canonicalizePath(normalizedPath0)
      )
    );

    let { pathname: effPathname } =
      splitPathAndQuery(effectivePath);

    let isProductEndpoint =
      effPathname === "/products" ||
      effPathname.startsWith("/products/");
    ...
    if (
      ...
      if (!isProductEndpoint) {
        effectivePath = canonicalizePath(
          sanitizeMetaPathForPublic(effectivePath)
        );
    ...
    if (
      ...
      effectivePath = canonicalizePath(
        normalizeMetaPath(effectivePath, {
          isPublic: true,
        })
      );
    ...
    if (
      !hasClientSecret &&
      isProductsList &&
      shouldForcePublicProductsList(effectivePath)
    ) {
      effectivePath = canonicalizePath(
        forcePublicProductsListPath(effectivePath)
      );
    ...
    // Product requests do not launch taxonomy-cache warming.
    // Metadata endpoints fill their own caches when requested.
    ...
    const result = await runDedupe(
      ...
      async () => {
        try {
          ...
          let res = await fetchUpstreamResilient(
            target,
            baseHeaders,
            {
              deadlineAt:
          ...
          // Retry the same query once if metadata says products
          // exist but the returned page is empty.
          if (
            isProductsList &&
            isSuspectEmptyProductsResponse(data)
          ) {
            await sleep(120);
    ...
    if (!result?.ok) {
      ...
      // Retain the existing same-endpoint metadata recovery.
      if (!isProductEndpoint) {
        const anyMeta = lastGoodGet(
          metaAnyKeyFromPathname(effPathname)
        );
      ...
      // Product failures remain errors when no exact last-good
      // response exists. Do not return a false empty success.
      if (isProductEndpoint) {
        const publicStatus =
          Number(result?.status) === 504
            ? 504
            : 502;
        ...
        return jsonResponse(
          {
            ...
            message: "Strapi product request failed",
            details: result?.errorText || null,
            ms,
          },
          publicStatus,
      ...
      const payloadStr = JSON.stringify({
        ...
        degraded: true,
        reason: "PUBLIC_META_DEGRADED_EMPTY",
        ms,
      });
    ...
    if (!hasClientSecret) {
      ...
      // Retain existing profile cache entries. Product failure
      // recovery above uses only exact-request entries.
      if (
        isProductsList &&
        isGoodProductPayload
      ) {
